...
for length in range(min(len(phone_number_to_check), max_prefix_len_to_try), 1, -1):
    ...
    cursor.execute(
        "SELECT CodeOperateur FROM PlagesNumeros WHERE PlageTel = ?", (prefix,)
    )
    ...
    if result:
        ...
        logging.debug(
            f"Found op_code '{op_code}' for prefix '{prefix}' in PlagesNumeros"
        )
        break
...
if op_code:
    ...
    cursor.execute(
        "SELECT NomOperateur FROM Operateurs WHERE CodeOperateur = ?", (
            op_code,)
    )
    ...
    if op_result:
        op_name = op_result[0]
        logging.info(
            f"Operator for most specific prefix '{found_prefix_in_db}': {op_name} (Code: {op_code})"
        )
    else:
        logging.warning(
            f"Operator name not found for CodeOperateur '{op_code}' (from prefix '{found_prefix_in_db}')"
        )
else:
    logging.info(
        f"No operator code found in PlagesNumeros for prefixes derived from {phone_number_to_check}."
    )
    logging.debug("\nDEBUG: Sample of PlagesNumeros for '07%':")
    cursor.execute(
        "SELECT PlageTel, CodeOperateur FROM PlagesNumeros WHERE PlageTel LIKE '07%' LIMIT 20"
    )
