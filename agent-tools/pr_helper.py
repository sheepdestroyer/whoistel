...
def run_gh_api(path, paginate=True):
    ...
    try:
        ...
        print(
            f"Error decoding JSON from GitHub API at {path}", file=sys.stderr)
        return []
...
def cmd_trigger(args):
    ...
    try:
        subprocess.run(
            ["gh", "pr", "comment", str(
                args.pr_number), "--body", "/gemini review"],
            check=True,
        )
        subprocess.run(
            [
...
def cmd_monitor(args):
    ...
    print(
        f"Monitoring PR #{args.pr_number} for activity since {args.since}...",
        file=sys.stderr,
    )

    if args.initial_wait > 0:
        print(
            f"Waiting {args.initial_wait}s before first check...", file=sys.stderr)
            ...
            if args.output:
                try:
                    with open(args.output, "w") as f:
                        json.dump(new_items, f, indent=2)
                    print(
                        f"Successfully written to {args.output}", file=sys.stderr)
                except IOError as e:
                    print(
                        f"Error writing to {args.output}: {e}", file=sys.stderr)
                    sys.exit(1)
            else:
                print(json.dumps(new_items, indent=2))
            sys.exit(0)
...
def cmd_verify(args):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(
                f"  STATUS: FAIL - Tests failed (exit code {result.returncode})",
                file=sys.stderr,
            )
    ...
    if not os.path.exists(args.file):
        print(
            f"\nNote: JSON feedback file '{args.file}' not found. Skipping heuristic checks.",
            file=sys.stderr,
        )
        return
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Unified PR Review Cycle Helper")
    parser.add_argument(
    ...
    p_trigger = subparsers.add_parser(
        "trigger", help="Trigger new reviews from bots")
    p_trigger.add_argument("pr_number", type=int)

    # Fetch
    p_fetch = subparsers.add_parser(
        "fetch", help="One-shot fetch of new feedback")
    p_fetch.add_argument("pr_number", type=int)
    p_fetch.add_argument(
        "--since", default="1970-01-01T00:00:00Z", help="ISO 8601 timestamp"
    )
    p_fetch.add_argument("--output", help="File path to write JSON results")

    # Monitor
    p_monitor = subparsers.add_parser(
        "monitor", help="Poll for new feedback until timeout"
    )
    p_monitor.add_argument("pr_number", type=int)
    p_monitor.add_argument(
        "--since",
        default="1970-01-01T00:00:00Z",
        help="ISO 8601 timestamp to filter new feedback from",
    )
    ...
    p_verify = subparsers.add_parser(
        "verify", help="Heuristic verification of local fixes"
    )
    p_verify.add_argument(
        "file", help="JSON file containing comments (from fetch/monitor)"
    )
