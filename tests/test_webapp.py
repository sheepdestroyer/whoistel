...
import whoistel
from webapp import MAX_COMMENT_LENGTH, create_app
...
def test_check_number_invalid_number_validation_error(client):
    ...
    response = client.post(
        "/check", data={"number": "invalid"}, follow_redirects=True)
    assert response.status_code == 200
...
def test_report_non_spam_flow(client):
    ...
    rv = client.post(
        "/report",
        data={"number": "0123987654", "date": "2023-02-01",
...
def test_database_connection_error(client):
    ...
    with patch(
        "whoistel.setup_db_connection", side_effect=whoistel.DatabaseError("Fail")
    ):
        rv = client.get("/view/0123456789")
...
def test_report_comment_truncation(client):
    ...
    )  # Ensure it's longer than MAX_COMMENT_LENGTH
...
def test_csrf_protection(client_with_csrf):
    ...
    rv = client_with_csrf.post(
        "/report", data={"number": "0123456789", "is_spam": "on"}
    )
    # Should return 400 Bad Request (CSRF token missing)
    assert rv.status_code == 400

    # Try to post without a CSRF token to /check
    rv = client_with_csrf.post("/check", data={"number": "0123456789"})
    # Should return 400 Bad Request (CSRF token missing)
    assert rv.status_code == 400
