...
def test_setup_database_removes_existing():
    ...
    with patch('generatedb.os.path.exists') as mock_exists,          patch('generatedb.os.remove') as mock_remove,          patch('generatedb.sqlite3.connect') as mock_connect:
...
def test_setup_database_creates_schema(tmp_path):
    ...
    try:
        ...
        assert expected_geo.issubset(columns_geo), f"Missing columns in PlagesNumerosGeographiques: {expected_geo - columns_geo}"

        # Verify Columns for Communes
        cursor.execute("PRAGMA table_info(Communes);")
        columns_communes = {row[1] for row in cursor.fetchall()}
        expected_communes = {'CodeInsee', 'NomCommune', 'CodePostal', 'NomDepartement', 'Latitude', 'Longitude'}
        assert expected_communes.issubset(columns_communes), f"Missing columns in Communes: {expected_communes - columns_communes}"
