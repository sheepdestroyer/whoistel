...
def import_operateurs(conn):
    ...
    try:
        ...
        df = pd.read_csv(
            "arcep/identifiants_ce.csv", sep=";", encoding="cp1252", dtype=str
        )
...
def import_numeros(conn):
    ...
    try:
        ...
        df = pd.read_csv("arcep/majournums.csv", sep=";",
                         encoding="cp1252", dtype=str)

        # Rename columns for clarity
        df.rename(
            columns={"EZABPQM": "PlageTel", "Mnémo": "CodeOperateur"}, inplace=True
        )
        ...
        df_geo.to_sql(
            "PlagesNumerosGeographiques", conn, if_exists="append", index=False
        )
        ...
        df_non_geo.to_sql("PlagesNumeros", conn,
                          if_exists="append", index=False)
        logger.info(
            f"Imported {len(df_non_geo)} non-geographic number ranges.")
