...
def get_operator_info(conn, code_operateur):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT NomOperateur, TypeOperateur, MailOperateur, SiteOperateur FROM Operateurs WHERE CodeOperateur=?",
        (code_operateur,),
    )
    row = cursor.fetchone()
    if row:
...
def get_commune_info(conn, code_insee):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT NomCommune, CodePostal, NomDepartement, Latitude, Longitude FROM Communes WHERE CodeInsee=?",
        (code_insee,),
    )
    row = cursor.fetchone()
    if row:
...
def search_number(conn, tel):
    ...
    for length in range(min(len(tel), 9), 1, -1):
        ...
        if is_geo:
            cursor.execute(
                f"SELECT CodeOperateur, CodeInsee FROM {table} WHERE PlageTel=?",
                (prefix,),
            )
            row = cursor.fetchone()
            if row:
            ...
            cursor.execute(
                f"SELECT CodeOperateur FROM {table} WHERE PlageTel=?", (
                    prefix,)
            )
            row = cursor.fetchone()
            if row:
...
def get_full_info(conn, tel):
    ...
    if not info:
        result["error"] = (
            "Numéro inconnu dans la base ARCEP (pas d'opérateur assigné trouvé)."
        )
        return result
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Outil de recherche d'informations sur les numéros de téléphone français (ARCEP)."
    )
    parser.add_argument(
        "numero", help="Numéro de téléphone à rechercher (ex: 0123456789, +33612345678)"
    )
    ...
    if not is_valid_phone_format(cleaned_number):
        if cleaned_number and cleaned_number != raw_tel:
            print(
                f"Erreur: Le numéro «{raw_tel}» est invalide après normalisation («{cleaned_number}»). Il doit contenir exactement 10 chiffres.",
                file=sys.stderr,
            )
        else:
            print(
                f"Erreur: Le numéro «{raw_tel}» est invalide. Il doit contenir exactement 10 chiffres.",
                file=sys.stderr,
            )
        sys.exit(1)
